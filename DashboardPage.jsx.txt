...
// TODO (step 5 - App.jsx): import { C, S, fmtRWF, toDay } from "../utils/helpers";
// TODO (step 5 - App.jsx): import { calcPnL, calcCapitalBalance, getMarketPrice } from "../utils/finance";
// TODO (step 5 - App.jsx): import { FS_FARM_DOC, getOnlineFarmData } from "../firebase";
// TODO (step 7 - Components): import AIPrediction from "../components/AIPrediction";

/**
 * DashboardPage — extracted from FarmIQ index.html (AHome function)
 * Props:
...
export default function DashboardPage({
  pigs, feeds, sales, logs, users, expenses, incomes,
  reproductions, stock, allData, setPage, capital, setCapital,
}) {
  ...
  // Monthly profit data (last 6 months) for bar chart
  const monthlyMap = {};
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        ...
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: C.text, letterSpacing: -0.4 }}>Farm Overview</div>
      ...
      <div style={{ background: "linear-gradient(145deg,#071410 0%,#0e2213 55%,#102616 100%)", borderRadius: 14, padding: "18px 22px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, boxShadow: "0 8px 28px rgba(0,0,0,.2)", position: "relative", overflow: "hidden" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 30, fontWeight: 800, color: capitalBalance >= 0 ? "#4ade80" : "#f87171", letterSpacing: -0.5 }}>{fmtRWF(capitalBalance)}</div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => { setShowCapitalSet(!showCapitalSet); setNewInitial(String(capital.initial || "")); }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 12, marginBottom: 16 }}>
        {[
          ...
          <div key={k.label} className="card-hover" style={{ background: k.bg, border: "1px solid " + k.border, borderRadius: 12, padding: "14px 14px 12px", cursor: "default", boxShadow: "0 1px 6px rgba(0,0,0,.04)" }}>
            ...
            <div style={{ fontSize: 10, color: C.faint, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 3 }}>{k.label}</div>
      ...
      <div style={S.g2}>
        {/* TODO (step 7): replace with <AIPrediction /> component */}
